...
public class ArchiveController {
  ...
  @Autowired
  private ArchiveFallbackFeign archiveFallbackFeign;
  ...
  public ModelAndView toArchive(HttpSession session, HttpServletRequest request) {
    ...
    List<Archive> archives = archiveFallbackFeign.selectAllArchiveGroup(); // 获取归档分组信息
    modelAndView.addObject("archives", archives);
    String date=  archives.get(0).getMonths();
    modelAndView.addObject("date",date);
    List<Article> oneArticle = archiveFallbackFeign.selectArticleByarchiveTime(date); //默认查出第一个日期
  ...
  public String archiveData(String date) {
    ...
    List<Article> articles = archiveFallbackFeign.selectArticleByarchiveTime(date);
